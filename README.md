...
This script submits a training job directly to Amazon SageMaker, bypassing the Clean Rooms ML workflow. It packages `train.py`, uploads it to S3, and launches a SageMaker training job using the scikit-learn managed container. Use this to iterate on the model independently of the Clean Rooms collaboration setup.

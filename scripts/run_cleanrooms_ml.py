...
def get_configured_table_associations(membership_id):
    ...
    for a in assocs.get("configuredTableAssociationSummaries", []):
        if "bank" in a["name"]:
            result["bank"] = a["arn"]
        elif "payment_processor" in a["name"]:
            result["payment_processor"] = a["arn"]
    if len(result) != 2:
...
def create_ml_input_channel(membership_id, collab_id, table_assocs, channel_name, channel_purpose):
    ...
    resp = crml.create_ml_input_channel(
        ...
        name=channel_name,
        description=f"{channel_purpose} input channel for fraud propensity model",
        inputChannel={
            "dataSource": {
                "protectedQueryInputParameters": {
                    "sqlParameters": {
                        "queryString": (
                            'SELECT DISTINCT b.account_id, b.login_count, b.failed_auth_attempts, '
                            'b.account_age_days, b.linked_devices, b.avg_transaction_value, '
                            'b.geo_spread_score, b.night_activity_ratio, b.avg_session_duration_min, '
                            'b.ip_change_frequency, b.dormant_reactivation, b.observation_date, '
                            'p.card_type, p.chargeback_count, p.declined_transactions, '
                            'p.transaction_velocity, p.merchant_category_diversity, '
                            'p.cross_border_ratio, p.days_since_last_dispute, p.last_dispute_date, '
                            'p.avg_txn_amount, p.txn_amount_stddev, p.weekend_txn_ratio, '
                            'p.rapid_succession_count, p.unique_country_count, '
                            'p.is_suspicious '
                            'FROM bank_association b '
                            'INNER JOIN payment_processor_association p ON b.customer_id = p.customer_id'
                        ),
                    }
                }
            },
...
def create_trained_model(membership_id, collab_id, algo_assoc_arn, training_channel_arn):
    ...
    resp = crml.create_trained_model(
        ...
        description="Fraud propensity scoring trained model",
...
def main():
    print("=" * 60)
    print("AWS Clean Rooms ML — FSI Fraud Train & Inference")
    print("=" * 60)
